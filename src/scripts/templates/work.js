...
if($('body').hasClass('page-template-work')){
    $('form select').select2({
        dropdownParent: $('.filters-wrap'),
        width: "100%",
        minimumResultsForSearch: -1,
    });

    const $worksForm = $("#works-form");
    const $worksResponse = $("#works-response");
    const $worksPageInput = $("#works-page-input");
    const $worksFilters = $worksForm.find('select, input');
    var currentPageSession;

    if ($(window).width() < 768) {
        $( ".filters-wrap input" ).prop( "disabled", true );
    }
    else {
        $( ".filters-wrap select" ).prop( "disabled", true );
    }
    
    function filterWorks() {
        $worksResponse.addClass("loading");
        ...
        setTimeout(function () {
            $.ajax({
                ...
                success: function (data) {
                    document.getElementById("works-response").innerHTML = data;
    
                    $('html, body').animate({
                        scrollTop: $("#works-response").offset().top
                    }, 500);
                },
                complete: function (data) {
                    $worksResponse.removeClass("loading");
    
                    currentPageSession = parseInt(sessionStorage.getItem("currentPage", page));
                    $('.pagination-pages').twbsPagination({
                        totalPages: $('.pagination-wrap').data('pages'),
                        visiblePages: 3,
                        initiateStartPageClick: false,
                        startPage: currentPageSession,
                        onPageClick: function (evt, page) {
                            $worksPageInput.val(page);
                            filterWorks();
    ...
    // $worksResponse.on("click", ".pagination-page", function () {
    //     $worksPageInput.val($(this).data("page"));
    //     filterWorks();
    // });
    
    $(document).on('click', '.next-page', function(){ 
        $('.page-item.next').trigger('click')
    })
    
    $(document).on('click', '.prev-page', function(){ 
        $('.page-item.prev').trigger('click')
    })
    
    $worksResponse.on("click", ".pagination-number", function () {
        $worksPageInput.val($(this).data("page"));
        filterWorks();
    });
    
    $worksFilters.on('change', function() {
        $worksPageInput.val(1);
        filterWorks();
